...
export default {
  ...
  async mounted() {
    this.$nextTick(() => {
      this.search({ ...this.$refs.search.getQuery(), systemType: 'F' })
    })
  },
  methods: {
    async search(query) {
      await this.loadingContainer(async () => {
        ...
        this.loginTypeLabel = data.loginTypeCountList.map(x => x.loginType)
        this.loginTypeData = data.loginTypeCountList.map(x => x.count)
    ...
    async pdf(query) {
      ...
      PDF.addImage(searchData, 'JPEG', 10, 15, 800, 35)

      let pageData = canvas.toDataURL('image/jpeg', 1.0)

      PDF.addImage(pageData, 'JPEG', -40, 105, 630, 530)
      PDF.save('前台使用軌跡統計.pdf')
    },
    async excel(query) {
      await this.loadingContainer(async () => {
        ...
        this.$f.toExcel(res, 'reportExcel.pdf')
      })
    }
  }
}
